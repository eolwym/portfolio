window.addEventListener('DOMContentLoaded', () => {
    ...
    addTechnologyBtn.addEventListener('click', async () => {
        fetch('http://localhost:3000/admin/technologies/form')
    ...
    editBtn.forEach(btn => {
        btn.addEventListener('click', (e)=> {
            ...
            fetch('http://localhost:3000/admin/technologies/form/update/' + id)
            .then(res => {return res.text()})
            .then(body => {
                form.innerHTML = body
                const close = document.querySelector('.close')
    ...
    deleteBtn.forEach(btn => {
        btn.addEventListener('click', (e)=> {
            ...
            if (confirm == true) {
                const elem = e
                const id = e.target.id
                fetch('http://localhost:3000/admin/technologies/delete/' + id, {
                    method:"POST"
                })
                .then(res => {
                    if (res.status = 200) {
