...
const public_users_queries_1 = require("../queries/public/public.users.queries");
...
passport_1.default.deserializeUser(async (id, done) => {
    try {
        const user = await public_users_queries_1.findUserPerId(id);
        done(null, user);
    }
    catch (e) {
        done(e);
...
passport_1.default.use('local', new LocalStrategy({
    ...
    try {
        const user = await public_users_queries_1.findUserPerEmail(email);
        if (user) {
